...
class NepalkhabarSpider(scrapy.Spider):
    name = "nepalkhabar_auto"
    allowed_domain = "nepalkhabar.com/auto-home"
    start_urls = ["https://nepalkhabar.com/auto-home"]

    def parse(self, response):
        main_links = response.xpath('//ul[@class="uk-navbar-nav uk-margin-remove"]/li/a/@href').extract()
        for main_link in main_links:
            main_link = response.urljoin(main_link)
            category = response.xpath('.//ul[@class="uk-navbar-nav uk-margin-remove"]/li/a/text()')[0].extract()
            yield scrapy.Request(main_link, callback=self.parse_link, meta={'category': category})
